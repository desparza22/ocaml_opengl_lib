...
module Surface = struct
  ...
  let refract_by_norm dir_in surface_norm refractive_index_in refractive_index_out =
    ...
    Vec.add refracted_perpendicular refracted_parallel
  ...
  module Material = struct
    ...
    let water_reflection_scale dot_val =
      Helper.clamp dot_val ~low:0. ~high:1.
      |> (fun dot_val -> dot_val ** 9.)(*
      |> (fun dot_val ->
        dot_val ** 6. *. 7. /. 9. +.
          (diffuse_reflection_scale dot_val *. 1. /. 9.))*)
    ...
    let reflection_probability_density
      ...
      | Water ->
         let light_in = Vec.neg light_in_flipped in
         let light_in_reflected = flip_by_norm light_in normal in
         let light_in_refracted = refract_by_norm light_in normal 1.0 (refractive_index Water) in
         let dot1 = Vec.dot light_in_reflected light_out in
         let dot2 = Vec.dot light_in_refracted light_out in
         Float.max (water_reflection_scale dot1 *. 0.8) (water_reflection_scale dot2 *. 0.8)
  end
  ...
  let refract_scatter (ray_in : Ray.t) (hit_record : Hit_record.t)
    ...
    Some (refract_by_norm ray_in.norm_direction hit_record.normal refractive_index_in refractive_index_out)
