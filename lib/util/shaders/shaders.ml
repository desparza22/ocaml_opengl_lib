...
module Blin_phong = struct
  ...
  let fragment_shader =
    "#version 330 core
     ...
     vec3 light_pos = vec3(10000., 0., 0.); //for now
     
     //ambient
     float power = 0.2;

     //diffuse
     vec3 light_camera_mid = (light_pos + camera_pos)/2;
     vec3 to_light = light_pos - v_frag;
     vec3 to_mid = light_camera_mid - v_frag;
     float dot_val = dot(to_light, to_mid);
     power += dot_val;

     //specular
     power = exp2(exp2(dot_val));
